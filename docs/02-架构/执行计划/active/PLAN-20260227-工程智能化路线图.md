...
| CAP-004 | 架构边界/依赖方向的结构化约束（taste invariants） | 缺少可机械校验的“目录/模块依赖规则” | `scripts/ci/arch-check.sh` + 规则文件（例如 `docs/02-架构/边界与依赖规则.md`） | CI 硬门禁：违规则阻断且报错提供修复指令 | Planned |
...
| CAP-007 | 技术债追踪（tech debt tracker） | 缺少统一技术债清单与偿还机制 | `docs/02-架构/技术债清单.md`（带 ID/优先级/owner/验收） | 每次计划评审更新状态；完成项移入 completed | Planned |
...
| CAP-009 | PR 闭环自动化（从失败到修复指路） | 缺少 PR 模板、失败指路一致性、修复剧本入口 | `.github/pull_request_template.md` + CI 报错信息标准化 | PR 默认携带 DoD；失败信息带下一步命令 | Planned |
